...
--type alias CodeTimeFunc =
--    {
--        instrCode: Int,
--        instrTime: CpuTimeCTime,
--        funcs: List (CpuTimeCTime -> Int -> Z80ROM -> Z80 -> Maybe Z80Transform)
--    }

--executeCB: Int -> Int -> CpuTimeCTime -> Z80ROM -> Z80Ram -> CodeTimeFunc
--executeCB code pc time rom48k ram =
--    let
--        param = mem (Bitwise.and (pc + 1) 0xFFFF) time rom48k ram
--    in
--       { instrCode = Bitwise.or 0xCB00 param.value, instrTime = param.time, funcs = cb_Funcs }
--
--executeDD: Int -> Int -> CpuTimeCTime -> Z80ROM -> Z80Ram -> CodeTimeFunc
--executeDD code pc time rom48k ram =
--    let
--        param = mem (Bitwise.and (pc + 1) 0xFFFF) time rom48k ram
--    in
--        { instrCode = Bitwise.or 0xDD00 param.value, instrTime = param.time, funcs = ix_Funcs }
--
--executeFD: Int -> Int -> CpuTimeCTime -> Z80ROM -> Z80Ram -> CodeTimeFunc
--executeFD code pc time rom48k ram =
--    let
--        param = mem (Bitwise.and (pc + 1) 0xFFFF) time rom48k ram
--    in
--        { instrCode = Bitwise.or 0xFD00 param.value, instrTime = param.time, funcs = iy_Funcs }
--
--executeDefault: Int -> Int -> CpuTimeCTime -> Z80ROM -> Z80Ram -> CodeTimeFunc
--executeDefault code pc time rom48k ram =
--    { instrCode = code, instrTime = time, funcs = standardFuncs }
--
--executeDict = Dict.fromList
--        [
--        (0xCB, executeCB)
--        ,(0xDD, executeDD)
--        ,(0xFD, executeFD)
...
execute_instruction rom48k z80 =
   ...
   case result of
       ...
       Transformer z80Transform index ->
           --let
           --    oldValue = z80.debugDict |> Dict.get index |> Maybe.withDefault 0
           --    z80_d = { z80 | debugDict = z80.debugDict |> Dict.insert index (oldValue + 1) }
           --in
