...
executeAndApplyDelta opCode cpuClock iff rom48k z80clock =
    ...
    in
    z80_core |> applyCoreChange coreChange clockTime pc_inc z80clock.pc


applyCoreChange : CoreChange -> CpuTimeCTime -> PCIncrement -> Int -> Z80Core -> Z80CoreWithClockTime
applyCoreChange coreChange clockTime pc_inc pc z80_core =
    let
        rawPc =
            case pc_inc of
                IncrementByOne ->
    ...
    case coreChange of
        ...
        MainOnly z80_main ->
            { core = { z80_core | main = z80_main }, clockTime = clockTime, pc = pcAfter }

        FlagsOnly z80_flags ->
            { core = { z80_core | flags = z80_flags }, clockTime = clockTime, pc = pcAfter }
        ...
        Looper repeatPCOffset z80Core ->
            case repeatPCOffset of
                ...
                JumpBack ->
                    { pc = pc, core = z80Core, clockTime = clockTime }
        ...
        LooperWithDelay repeatPCOffset shortDelay z80Core ->
            case repeatPCOffset of
                ...
                JumpBack ->
                    { pc = pc, core = z80Core, clockTime = clockTime |> addExtraCpuTime shortDelay }
...
executeCoreInstruction rom48k pc z80_core =
    let
        ...
        clock =
            { core = z80_core, pc = pc, clockTime = clockTime }

        newClock =
            case ct of
                UncompiledOpcode opcode cpuTimeCTime ->
                    clock |> executeAndApplyDelta opcode cpuTimeCTime IFF_0 rom48k.z80rom
                ...
                Z80Compiled compiled ->
                    ...
                    in
                    z80_core |> applyCoreChange coreChange clockTime compiled.length pc
    in
                    ...
                    core_1_clock =
                        case cpuInstruction of
                            UncompiledOpcode opCode clockTime ->
                                clock |> executeAndApplyDelta opCode clockTime z80.iff rom48k.z80rom
                            ...
                            Z80Compiled compiled ->
                                let
                                    ...
                                    coreChange =
                                        core |> compiled.function clockTime rom48k.z80rom

                                    newClock =
                                        core |> applyCoreChange coreChange clockTime compiled.length clock.pc
                                in
                                newClock
                in
