...
set_flags : Int -> Z80Byte -> FlagRegisters
set_flags tmp_f a =
    let
        fr =
...
z80_add b the_flags =
    let
        ...
        fr =
            Bitwise.and ff 0xFF
    in
    { the_flags | fa = fa, fb = fb, ff = ff, fr = fr, a = fr |> intToz80byte }
...
z80_sub b flagRegs =
    let
        ...
        fr =
            Bitwise.and ff 0xFF
    in
    { flagRegs | fa = fa, fb = fb, ff = ff, fr = fr, a = fr |> intToz80byte }
...
z80_and b flagRegs =
    let
        fr =
            flagRegs.a |> z80byteToInt |> Bitwise.and b
        ...
        fa =
            complement a
    in
    { flagRegs | fa = fa, fb = 0, ff = ff, fr = fr, a = a |> intToz80byte }
...
z80_or b flagRegs =
    let
        fr =
            flagRegs.a |> z80byteToInt |> Bitwise.or b
...
z80_xor b flagRegs =
    let
        fr =
            flagRegs.a |> z80byteToInt |> Bitwise.xor b
...
cpl flagRegs =
    let
        new_a =
            flagRegs.a |> z80byteToInt |> Bitwise.xor 0xFF
    ...
    in
    { flagRegs | a = new_a |> intToz80byte, ff = ff, fb = fb, fa = fa }


inc : Z80Byte -> FlagRegisters -> Z80ByteWithFlags
inc v_byte flagRegs =
    let
        v = v_byte |> z80byteToInt
        ff =
    ...
    in
    Z80ByteWithFlags (vv|> intToz80byte) { flagRegs | ff = Bitwise.or ff vv, fb = 1, fa = v, fr = vv }
...
testBit testType v flagRegs =
    ...
    let
        m =
            testType |> bitMaskFromBit |> Bitwise.and (v |>  z80byteToInt)

        ff =
            Bitwise.or (Bitwise.and flagRegs.ff (complement 0xFF)) (Bitwise.or (Bitwise.and (v|>  z80byteToInt) c_F53) m)
...
rot a flagRegs =
    ...
    in
    { flagRegs | ff = ff, fb = fb, fa = fa, a = Bitwise.and a 0xFF |> intToz80byte }
...
scf_ccf x flagRegs =
    let
        ...
        ff =
            Bitwise.or (Bitwise.or (Bitwise.xor 0x0100 x) (Bitwise.and flagRegs.ff c_FS)) (flagRegs.a |> z80byteToInt |> Bitwise.and c_F53)
    in
...
daa flagRegs =
    let
        ...
        d0 =
            if Bitwise.or (flagRegs.a |> z80byteToInt) (Bitwise.and flagRegs.ff 0x0100) > 0x99 then
                0x0160
        ...
        d =
            if Bitwise.or (Bitwise.and (flagRegs.a |> z80byteToInt) 0x0F) h > 9 then
                d0 + 6
        ...
        fa =
            Bitwise.or (flagRegs.a |> z80byteToInt) 0x0100

        ( a0, fb ) =
            if Bitwise.and flagRegs.fb 0x0200 == 0 then
                ( (flagRegs.a |> z80byteToInt) + d, d )

            else
                ( (flagRegs.a |> z80byteToInt) - d, complement d )
    ...
    in
    { flagRegs | fr = fr, a = a |> intToz80byte, fb = fb, fa = fa, ff = ff }
...
get_af z80_flags =
    Bitwise.or (z80_flags.a |> z80byteToInt |> shiftLeftBy8) (get_flags z80_flags)
