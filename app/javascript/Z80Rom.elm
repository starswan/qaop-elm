...
type alias Z80ROM =
    { rom48k : Array (Array Int)
    , keyboard : Keyboard
    , z80ram : Z80Ram
    }


constructor : Array (Array Int) -> Z80ROM
...
getROMValue addr z80rom =
    let
        addr1 =
            addr |> shiftRightBy 10

        addr2 =
            addr |> Bitwise.and 0x03FF
    in
    case Array.get addr1 z80rom.rom48k of
        Just array ->
            case array |> Array.get addr2 of
                Just a ->
                    a

                Nothing ->
                    debugTodo "getROMValue" (String.fromInt addr1 ++ String.fromInt addr2) -1
...
parseRomFile bytes =
    ...
    case array of
        Just listint ->
            let
                arrays =
                    listint |> List.map (\list -> list |> Array.fromList) |> Array.fromList
            in
            Ok (constructor arrays)
