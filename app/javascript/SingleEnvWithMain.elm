...
applySingleEnvMainChange new_pc z80changeData z80_flags =
    ...
    case z80changeData of
        ...
        SingleEnvFlagFunc flagFunc int cpuTimeCTime ->
            { pcIncrement = new_pc, time = cpuTimeCTime, timeIncrement = FourTStates, operation = ChangeFlagRegisters (z80_flags |> changeFlags flagFunc int) }
        ...
        SingleEnvNewHL16BitAdd ixiyhl hl sp ctime ->
            let
                new_xy =
                    add16 hl sp z80_flags
                ...
                operation =
                    case ixiyhl of
                        IX ->
                            ChangeMainFlagsIX new_xy.value

                        IY ->
                            ChangeMainFlagsIY new_xy.value

                        HL ->
                            ChangeMainFlagsHL new_xy.value

                --new_time =
                --    ctime |> addCpuTimeTimeInc new_xy.time
            in
            { pcIncrement = new_pc, time = ctime, timeIncrement = ElevenTStates, operation = ChangeMainWithFlags operation new_xy.flags }
