        ...
        Just ( f, pcInc ) ->
            ...
            case f doubleParam.value16 of
                ...
                NewHLIndirect int ->
                    ...
                    in
                    Just { pcIncrement = new_pc, time = value.time, timeIncrement = TenTStates, operation = ChangeMain (ChangeHLRegister value.value16) }
                ...
                NewIXIndirect int ->
                    let
                        ...
                        value =
                            { env | time = doubleParam.time } |> mem16 int rom48k
                    in
                    Just { pcIncrement = new_pc, time = value.time, timeIncrement = TenTStates, operation = ChangeMain (ChangeIXRegister value.value16) }
                ...
                NewIYIndirect int ->
                    let
                        ...
                        value =
                            { env | time = doubleParam.time } |> mem16 int rom48k
                    in
                    Just { pcIncrement = new_pc, time = value.time, timeIncrement = TenTStates, operation = ChangeMain (ChangeIYRegister value.value16) }
