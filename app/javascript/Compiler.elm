...
import Z80Execute exposing (applyJumpChangeDelta, applySimpleTripleChangeDelta)
            ...
            let
                ...
                running =
                    if (input.seen |> Set.member key) || (input.compiled |> Dict.member key) then
                        ...
                        let
                            paramValue =
                                z80env |> getMem8 (Bitwise.and (key + 1) 0xFFFF) clockTime rom48k |> Tuple.first

                            maybeJump =
                                maybeRelativeJump |> Dict.get value |> Maybe.map (\( f, duration ) -> ( f paramValue, duration ))
                        in
                        case maybeJump of
                            Just ( relJump, duration ) ->
                                let
                                    dictVal =
                                        CompiledInstruction (\_ _ z80core -> z80core |> applyJumpChangeDelta relJump) duration IncrementByTwo
                                in
                                case relJump of
                                    ...
                                    Nothing ->
                                        ...
                                        case maybeLongJump of
                                            Just ( aLongJump, duration ) ->
                                                let
                                                    dictVal =
                                                        CompiledInstruction (\_ _ z80core -> z80core |> applySimpleTripleChangeDelta rom48k clockTime (TripleByteJumpChange aLongJump)) duration IncrementByThree
                                                in
                                                case aLongJump of
                                                    NewPCRegister _ ->
                                                        if (input.seen |> Set.member address) || (input.compiled |> Dict.member address) then
                    ...
                    case lengthAndDuration key rom48k z80env of
                        ...
                        Nothing ->
                            ...
                            if [ 0xF3, 0xFB, 0xD9, 0x33, 0x3B, 0x08, 0x76 ] |> List.member value then
                                ...
                                else if tripleMainRegsIYThree |> Dict.member nextValue then
...
compileRom rom48k z80env =
    let
        ...
        compiledResult =
            if result.compiled |> Dict.isEmpty then
                Dict.empty
...
createCompiledRom : Dict Int Int -> CompiledZ80ROM
createCompiledRom rom_dict =
    let
        z80rom =
            { rom48k = rom_dict, keyboard = Keyboard.constructor, z80ram = Z80Ram.constructor }
