module CB80Test exposing (..)

import Compiler exposing (createCompiledRom)
import Dict
import Expect exposing (Expectation)
import Test exposing (..)
import Triple
import Z80 exposing (executeCoreInstruction)
import Z80CoreWithClockTime
import Z80Env exposing (setMemWithTime)
import Z80Mem exposing (getMem8)
import Z80Rom


suite : Test
suite =
    let
        addr =
            0x5800

        sp =
            0xF765

        hl =
            0x1234

        clock =
            Z80CoreWithClockTime.constructor

        old_z80 =
            clock.core

        old_z80env =
            old_z80.env

        z80main =
            old_z80.main

        flags =
            old_z80.flags

        z80 =
            { old_z80 | env = { old_z80env | sp = sp }, main = { z80main | hl = hl } }

        z80env =
            { z80env = z80.env, time = clock.clockTime }

        z80rom =
            createCompiledRom Dict.empty
    in
    describe "Bit instructions (CB)"
        [ describe "RES 0,B"
            [ test "0xCB 80 RES 0,B" <|
                \_ ->
                    let
                        new_env =
                            z80env
                                |> setMemWithTime addr 0xCB
                                |> setMemWithTime (addr + 1) 0x80
                                |> .z80env

                        ( new_z80, new_pc ) =
                            executeCoreInstruction z80rom
                                addr
                                { z80
                                    | env = new_env
                                    , main = { z80main | b = 0xFF }
                                }
                                |> Triple.dropSecond
                    in
                    Expect.equal ( addr + 2, 0xFE ) ( new_pc, new_z80.main.b )
            , test "0xDD 0xCB d 0x80 RES 0 (IX + d), B" <|
                \_ ->
                    let
                        new_env =
                            z80env
                                |> setMemWithTime addr 0xDD
                                |> setMemWithTime (addr + 1) 0xCB
                                |> setMemWithTime (addr + 2) 0x45
                                |> setMemWithTime (addr + 3) 0x80
                                |> setMemWithTime 0x6545 0xFF
                                |> .z80env

                        ( new_z80, new_pc ) =
                            executeCoreInstruction z80rom
                                addr
                                { z80
                                    | env = new_env
                                    , main = { z80main | ix = 0x6500, b = 0xFF }
                                }
                                |> Triple.dropSecond

                        mem_value =
                            new_z80.env |> getMem8 0x6545 clock.clockTime z80rom.z80rom |> Tuple.first
                    in
                    Expect.equal ( addr + 4, 0xFE, 0xFE ) ( new_pc, new_z80.main.b, mem_value )
            , test "0xFD 0xCB d 0x80 RES 0 (IY + d), B" <|
                \_ ->
                    let
                        new_env =
                            z80env
                                |> setMemWithTime addr 0xFD
                                |> setMemWithTime (addr + 1) 0xCB
                                |> setMemWithTime (addr + 2) 0xFF
                                |> setMemWithTime (addr + 3) 0x80
                                |> setMemWithTime 0x6545 0xFF
                                |> .z80env

                        ( new_z80, new_pc ) =
                            executeCoreInstruction z80rom
                                addr
                                { z80
                                    | env = new_env
                                    , main = { z80main | iy = 0x6546, hl = 0x5000 }
                                }
                                |> Triple.dropSecond

                        mem_value =
                            new_z80.env |> getMem8 0x6545 clock.clockTime z80rom.z80rom |> Tuple.first
                    in
                    Expect.equal ( addr + 4, 0xFE, 0xFE ) ( new_pc, new_z80.main.b, mem_value )
            ]
        , test "0xCB 81 RES 0,C" <|
            \_ ->
                let
                    new_env =
                        z80env
                            |> setMemWithTime addr 0xCB
                            |> setMemWithTime (addr + 1) 0x81
                            |> .z80env

                    ( new_z80, new_pc ) =
                        executeCoreInstruction z80rom
                            addr
                            { z80
                                | env = new_env
                                , main = { z80main | c = 0xFF }
                            }
                            |> Triple.dropSecond
                in
                Expect.equal ( addr + 2, 0xFE ) ( new_pc, new_z80.main.c )
        , test "0xCB 82 RES 0,D" <|
            \_ ->
                let
                    new_env =
                        z80env
                            |> setMemWithTime addr 0xCB
                            |> setMemWithTime (addr + 1) 0x82
                            |> .z80env

                    ( new_z80, new_pc ) =
                        executeCoreInstruction z80rom
                            addr
                            { z80
                                | env = new_env
                                , main = { z80main | d = 0xFF }
                            }
                            |> Triple.dropSecond
                in
                Expect.equal ( addr + 2, 0xFE ) ( new_pc, new_z80.main.d )
        , test "0xCB 83 RES 0,E" <|
            \_ ->
                let
                    new_env =
                        z80env
                            |> setMemWithTime addr 0xCB
                            |> setMemWithTime (addr + 1) 0x83
                            |> .z80env

                    ( new_z80, new_pc ) =
                        executeCoreInstruction z80rom
                            addr
                            { z80
                                | env = new_env
                                , main = { z80main | e = 0xFF }
                            }
                            |> Triple.dropSecond
                in
                Expect.equal ( addr + 2, 0xFE ) ( new_pc, new_z80.main.e )
        , test "0xCB 84 RES 0,H" <|
            \_ ->
                let
                    new_env =
                        z80env
                            |> setMemWithTime addr 0xCB
                            |> setMemWithTime (addr + 1) 0x84
                            |> .z80env

                    ( new_z80, new_pc ) =
                        executeCoreInstruction z80rom
                            addr
                            { z80
                                | env = new_env
                                , main = { z80main | hl = 0xFFFF }
                            }
                            |> Triple.dropSecond
                in
                Expect.equal ( addr + 2, 0xFEFF ) ( new_pc, new_z80.main.hl )
        , test "0xCB 85 RES 0,L" <|
            \_ ->
                let
                    new_env =
                        z80env
                            |> setMemWithTime addr 0xCB
                            |> setMemWithTime (addr + 1) 0x85
                            |> .z80env

                    ( new_z80, new_pc ) =
                        executeCoreInstruction z80rom
                            addr
                            { z80
                                | env = new_env
                                , main = { z80main | hl = 0xFFFF }
                            }
                            |> Triple.dropSecond
                in
                Expect.equal ( addr + 2, 0xFFFE ) ( new_pc, new_z80.main.hl )
        , test "0xCB 0x86 RES 0, (HL)" <|
            \_ ->
                let
                    new_env =
                        z80env
                            |> setMemWithTime addr 0xCB
                            |> setMemWithTime (addr + 1) 0x86
                            |> setMemWithTime 0xA07E 0xFF
                            |> .z80env

                    ( new_z80, new_pc ) =
                        executeCoreInstruction z80rom
                            addr
                            { z80
                                | env = new_env
                                , main = { z80main | hl = 0xA07E, b = 0xA5 }
                            }
                            |> Triple.dropSecond

                    mem_value =
                        new_z80.env |> getMem8 0xA07E clock.clockTime z80rom.z80rom |> Tuple.first
                in
                Expect.equal ( addr + 2, 0xFE ) ( new_pc, mem_value )
        , test "0xCB 87 RES 0,A" <|
            \_ ->
                let
                    new_env =
                        z80env
                            |> setMemWithTime addr 0xCB
                            |> setMemWithTime (addr + 1) 0x87
                            |> .z80env

                    ( new_z80, new_pc ) =
                        executeCoreInstruction z80rom
                            addr
                            { z80
                                | env = new_env
                                , flags = { flags | a = 0xFF }
                                , main = { z80main | hl = 0xFFFF }
                            }
                            |> Triple.dropSecond
                in
                Expect.equal ( addr + 2, 0xFE ) ( new_pc, new_z80.flags.a )
        ]
