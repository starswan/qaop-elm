...
suite =
   let
       ...
       z80 = { old_z80 | env = { old_z80env | pc = addr, sp = sp }, main = { z80main | hl = hl } }
       flags = z80.flags
       z80env = z80.env
      ...
      [
         ...
         [
            ...
            \_ ->
               ...
               in
                  Expect.equal ((addr + 2), 0x40) (new_z80.env.pc, new_z80.flags.a)
            ...
            \_ ->
               let
                  ...
                  new_z80 = execute_instruction z80rom { z80 | env = new_env,
                                                        main = { z80main | hl = 0x6545 }, flags = { flags | a = 0x47 } }
               in
                  Expect.equal ((addr + 4), 0x0201) (new_z80.env.pc, new_z80.env.sp)
            ,test "LDIR ED B0" <|
            \_ ->
               let
                  new_env = z80env
               ...
               in
                  ...
                  {pc=new_z80.env.pc, b=new_z80.main.b, c=new_z80.main.c,e=new_z80.main.e,d=new_z80.main.d,hl=new_z80.main.hl, mem=mem_vals}
            ,test "0xED 78 IN A, (C)" <|
            \_ ->
               let
                  new_env = z80env
               ...
               in
                  Expect.equal {pc=(addr + 2), fr=0xFF, a=0xFF} {pc=new_z80.env.pc, fr=new_z80.flags.fr, a=new_z80.flags.a}
            ,test "0xE1 POP HL" <|
            \_ ->
               let
                  new_env = z80env
                  ...
                  new_z80 = execute_instruction z80rom { z80 | env = new_env,
                                                        main = { z80main | hl = 0x6545, b = 0xA5, c = 0x5F }, flags = { flags | a = 0x39 } }
               in
                  Expect.equal ((addr + 1), 0x3445, sp + 2) (new_z80.env.pc, new_z80.main.hl, new_z80.env.sp)
            ...
            \_ ->
               ...
               in
                  Expect.equal ((addr + 2), 0x3445, sp + 2) (new_z80.env.pc, new_z80.main.iy, new_z80.env.sp)
            ,test "0xE3 EX (SP),HL" <|
            \_ ->
               let
                  new_env = z80env
               ...
               in
                  Expect.equal
                  {pc=new_z80.env.pc, sp=new_z80.env.sp, hl=new_z80.main.hl, top=(new_z80.env |> mem16 sp z80rom).value}
            ...
            \_ ->
               ...
               in
                  Expect.equal
                  {pc=new_z80.env.pc, sp=new_z80.env.sp, ix=new_z80.main.ix, top=(new_z80.env |> mem16 sp z80rom).value}
            ...
            \_ ->
               ...
               in
                  ...
                  {pc=new_z80.env.pc, hl=new_z80.main.hl, d=new_z80.main.d, e =new_z80.main.e}
            ,test "0xE5 PUSH HL" <|
            \_ ->
               let
                  new_env = z80env
               ...
               in
                  Expect.equal
                  {pc=new_z80.env.pc, sp=new_z80.env.sp, top=(new_z80.env |> mem16 (sp - 2) z80rom).value}
